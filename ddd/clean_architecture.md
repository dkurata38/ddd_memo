...
使っているフレームワークが自分の思っている通りのアップデートをするとは限らない
フレームワークを将来乗り換える可能性もある
DIもビジネスロジックにアノテーションをつける形式ではなく、
Mainコンポーネントで注入するのが望ましい

### 第33章

アクターとユースケースを分析する。
単一責任の原則と依存性のルールによる分割を行う。
変更の理由に対応するのはアクターによる分割、変更の頻度に対応するのは方針のレベルの違いによる分割である。

### 補足

良い設計でも実装方法が複雑すぎると設計が崩れてしまう。
選択肢を残しておく必要はあるが、
チームの規模、チームのスキル、ソリューションの複雑さ、予算などを考慮する必要もある。
